...
void AES::CheckLength(unsigned int len) {
    if (len % blockBytesLen != 0) {
        std::cout << "\n\nin AES::CheckLength(), len is: " << len 
                  << ", but blockBytesLen is: " << blockBytesLen 
                  << " = " << len / blockBytesLen << ", R: " << len % blockBytesLen << "\n\n\n";
        throw new std::logic_error("Not divisible error");
    }
}
...
unsigned char *AES::VectorToArray(std::vector<unsigned char>& a) {
    unsigned char* pbuf = new unsigned char[a.size()];
    std::copy(a.begin(), a.end(), pbuf);
    return pbuf;
}
...
std::vector<unsigned char> AES::EncryptECB(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = EncryptECB(VectorToArray(in), (unsigned int)in.size(),
                     VectorToArray(key));
...
std::vector<unsigned char> AES::DecryptECB(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = DecryptECB(VectorToArray(in), (unsigned int)in.size(),
                     VectorToArray(key));
...
std::vector<unsigned char> AES::EncryptCBC(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = EncryptCBC(VectorToArray(in), (unsigned int)in.size(),
...
std::vector<unsigned char> AES::DecryptCBC(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = DecryptCBC(VectorToArray(in), (unsigned int)in.size(),
                     VectorToArray(key), VectorToArray(iv));
...
std::vector<unsigned char> AES::EncryptCFB(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = EncryptCFB(VectorToArray(in), (unsigned int)in.size(),
                     VectorToArray(key), VectorToArray(iv));
...
std::vector<unsigned char> AES::DecryptCFB(std::vector<unsigned char> in,
    ...
    unsigned char *out 
        = DecryptCFB(VectorToArray(in), (unsigned int)in.size(),
                     VectorToArray(key), VectorToArray(iv));
