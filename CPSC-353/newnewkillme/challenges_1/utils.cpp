...
unsigned char tobase64(unsigned char ascii_char) { 
  ...
  if (c < 58) { return c - '0' + 52; }  // '0' --> 52
  if (c < 91) { return c - 'A'; }       // 'A' --> 0
  return c - 'a' + 26;                  // 'a' --> 26
}
...
void to_ascii_from_word(const char* src, std::string& dst) {
  ...
  unsigned mask = 127;   // mask needs to get only 8 bits at a time
...
std::string find_single_char_xor(const std::string& src, unsigned char& maxkey,
                          int& maxscore) {
  std::string hexsrc, key_xor, message;

  hex_decode(src, hexsrc);
  ...
  for (unsigned char key = ' '; key < 'z'; ++key) { 
      ...
      single_char_xor(hexsrc, key_xor, key);
...
std::string find_string_single_char_xor(const std::vector<std::string>& sources, int& index,
  ...
  for (const std::string& src : sources) { 
    ...
    poss_message = find_single_char_xor(src, key, score);
...
void test_get_repeating_key_xor(const std::string& src) { 
  ...
  std::string dst, hexdst, transposed;
  unsigned int keysize = 0;

  to_ascii_from_string(src, dst);


  std::vector<hamming_dist_key> best_keys = best_keysize_hamming_distance(dst);

  for (const hamming_dist_key& hdk : best_keys) { 
    std::cout << hdk << "\n";
  }

    transpose_blocks(dst, keysize, transposed);
    std::string key = get_repeating_key_xor(transposed, keysize);

    // std::string key = std::string("TerminatortX: Bring the noise");
    std::string x_or, plaintext;
    repeating_key_xor(dst, key, plaintext);
